import pygame, sys, math, random, textwrap, pickle, numpy as np
...
def place_objects(room):
    ...
    max_monsters = from_dungeon_level([[2, 1], [3, 4], [5, 6]])  # 2 3 5 TODO:Fix mob spawn rate after testings

    # chance of each monster
    monster_chances = {}
    monster_chances['orc'] = 80  # orc always shows up, even if all other monsters have 0 chance
    monster_chances['troll'] = from_dungeon_level([[15, 3], [30, 5], [60, 7]])  # 15 30 60
...
def new_game():
    ...
    player = Object(TILE_WIDTH * 10, TILE_HEIGHT * 7, images[2], "player",
                    blocks=True, fighter=fighter_component, ai=player_ai)
...
def play_game():
    ...
    while True:
        ...
        for event in pygame.event.get():  # wait for events
            ...
            if event.type == KEYDOWN:
                if event.key == K_UP:
                    if game_state == 'playing' and player_action != 'taked-turn':
                        for entity in active_entities:
                            if entity.ai and entity.fighter.owner.name == 'player':
                                entity.ai.take_turn("up")
                if event.key == K_DOWN:
                    if game_state == 'playing' and player_action != 'taked-turn':
                        for entity in active_entities:
                            if entity.ai and entity.fighter.owner.name == 'player':
                                entity.ai.take_turn("down")
                if event.key == K_RIGHT:
                    if game_state == 'playing' and player_action != 'taked-turn':
                        for entity in active_entities:
                            if entity.ai and entity.fighter.owner.name == 'player':
                                entity.ai.take_turn("right")
                if event.key == K_LEFT:
                    if game_state == 'playing' and player_action != 'taked-turn':
                        for entity in active_entities:
                            if entity.ai and entity.fighter.owner.name == 'player':
                                entity.ai.take_turn("left")
...
def auxFunction():
    player_x, player_y = player.tile.x, player.tile.y
    path = a_star_path_finder()
    path.reverse()
    path.pop(0)
    for i in range(1, len(path)):
        actual = path[i]
        previous = path[i - 1]
        x_dir = actual.x - previous.x
        y_dir = actual.y - previous.y
        if x_dir < 0:
            print("Esquerda")
        if x_dir > 0:
            print("Direita")
...
def allCoordinatesAux():
    allCoordinatesOpen = []
    for x in range(len(level_map)):
        for y in range(len(level_map[x])):
            if not level_map[x][y].blocked:
                allCoordinatesOpen.append(level_map[x][y])
    return allCoordinatesOpen


def stairsCoordinates():
    allCoordinatesOpen = allCoordinatesAux()
    for x in range(len(allCoordinatesOpen)):
        if allCoordinatesOpen[x].item is not None and allCoordinatesOpen[x].item.name == 'stairs':
            return allCoordinatesOpen[x]
...
def a_star_path_finder():
    ...
    open = []
    closed = []
    starting_position = Position(player.tile, 0, 0)  # posição do player
    final_position = Position(stairsCoordinates(), 0, 0)  # posição das escadas
    closed.append(starting_position)
    open.extend(getNeighbors(starting_position, final_position))  # vizinhos da posição inicial (posição do player)
    while True:
        print(aux)
        open.sort(key=get_f)
        current = open[0]
        ...
        for neighbor in getNeighbors(current, final_position):
            if neighbor not in closed:
                for neighbor_to_substitute in open:
                    if neighbor == neighbor_to_substitute:
                        neighbor_to_substitute = neighbor
                open.append(neighbor)
        aux += 1


def getPath(final_position):
    path = []
    current_position = final_position.parent
    path.append(Position(stairsCoordinates()))
    path.append(final_position)
    while current_position.parent is not None:
        current_position = current_position.parent
        path.append(current_position.parent)
    return path


def getNeighbors(centerPos, finalPos):
    allPositions = allCoordinatesAux()
    neighbors = []
    for potential_neighbor in allPositions:
        if checkNeighbor(centerPos, potential_neighbor):
            g = int(math.sqrt(
                math.pow((centerPos.x - potential_neighbor.x), 2) + math.pow((centerPos.y - potential_neighbor.y),
                                                                             2)) + centerPos.g)
            h = int(math.sqrt(
                math.pow((finalPos.x - potential_neighbor.x), 2) + math.pow((finalPos.y - potential_neighbor.y), 2)))
            neighbors.append(Position(potential_neighbor, g, h, centerPos))
    return neighbors


def checkNeighbor(actualPos, potential_neighbor):
    if (potential_neighbor.x == actualPos.x + TILE_WIDTH and potential_neighbor.y == actualPos.y) \
            or (potential_neighbor.x == actualPos.x - TILE_WIDTH and potential_neighbor.y == actualPos.y) \
            or (potential_neighbor.x == actualPos.x and potential_neighbor.y == actualPos.y + TILE_HEIGHT) \
            or (potential_neighbor.x == actualPos.x and potential_neighbor.y == actualPos.y - TILE_HEIGHT):
        return True
    return False
...
class Position:
    ...
    def __init__(self, tile, g=0, h=0, parent=None):
        self.x = tile.x
        self.y = tile.y
        self.g = g  # distancia ao node original (posição original do player)
        self.h = h  # distancia ao node final (escadas)
        if tile.entity:
            self.m = 32
        else:
            self.m = 0
        self.f = g + h + self.m  # soma das heuristicas
        self.parent = parent  # parent para ajudar a calcular o path
...
class PlayerAi:
    def take_turn(self, move):
        if self.isHPBelowHalf():
            self.isPotionAvailable()
        self.checkMonstersAround()
        if move == 'up':
            player_move_or_attack(0, -TILE_HEIGHT)
        elif move == 'down':
            player_move_or_attack(0, TILE_HEIGHT)
    ...
    def isHPBelowHalf(self):
        if player.fighter.hp < (player.fighter.max_hp / 2):
            return True
        return False

    def isPotionAvailable(self):
        for item in inventory:
    ...
    def checkMonstersAround(self):
        player_tile_x, player_tile_y = int(player.x / TILE_WIDTH), int(player.y / TILE_HEIGHT)
        if level_map[player_tile_x - 1][player_tile_y - 1].entity is not None:
            print(level_map[player_tile_x - 1][player_tile_y - 1].entity.name)
        else:
            print("up left")
        if level_map[player_tile_x][player_tile_y - 1].entity is not None:
            print(level_map[player_tile_x][player_tile_y - 1].entity.name)
        else:
            print("up center")
        if level_map[player_tile_x + 1][player_tile_y - 1].entity is not None:
            print(level_map[player_tile_x + 1][player_tile_y - 1].entity.name)
            ...
            print("up right")
        if level_map[player_tile_x - 1][player_tile_y].entity is not None:
            print(level_map[player_tile_x - 1][player_tile_y].entity.name)
        else:
            print("left")
        if level_map[player_tile_x + 1][player_tile_y].entity is not None:
            print(level_map[player_tile_x + 1][player_tile_y].entity.name)
    ...
    def checkItem(self):
        if player.tile.item and player.tile.item.item and player.tile.item.name == 'healing potion':
            player.tile.item.item.pick_up()
            player.tile.item = None
